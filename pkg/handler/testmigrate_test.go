...
// where every setup*TestHandler helper in this package is invoked against one fixed, shared
// DSN (configured via env vars for the whole job), so without memoizing this, every setup call
// in this package redundantly resets the same real postgres/mysql database. Locally, sqlite
// tests get a unique per-call DSN (see util.GetTestDBConfig), so each call is a genuinely new,
// unmigrated database and this cache never hits — behavior there is unchanged.
