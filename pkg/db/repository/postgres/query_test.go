...
func TestPSQLQuerier_AuditLogOperations(t *testing.T) {
	querier := &PSQLQuerier{}
	ctx := context.Background()
	...
	t.Run("Insert", func(t *testing.T) {
		...
		if !strings.Contains(sql, "INSERT INTO \"ezauth_audit_logs\"") {
			t.Errorf("unexpected SQL: %s", sql)
		}
		if len(args) < 3 {
	...
	t.Run("ListByUserID", func(t *testing.T) {
		...
		if !strings.Contains(sql, "SELECT") || !strings.Contains(sql, "FROM \"ezauth_audit_logs\"") {
			t.Errorf("unexpected SQL: %s", sql)
		}
	...
	t.Run("ListByUserID with filters", func(t *testing.T) {
		...
		if !strings.Contains(sql, "\"event_type\" = $2") {
